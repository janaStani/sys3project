      ...
      <div style={{ background:"#1a1c22", border:"1px solid #333", borderRadius:16, width:"100%", maxWidth:460 }}>
        ...
        <div style={{ padding:"0 20px 20px", display:"flex", flexDirection:"column", gap:14 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <Field label="Make">
              <select value={form.make} onChange={e => setField("make", e.target.value)} style={INPUT_STYLE}>
                {MAKES.map(m => <option key={m}>{m}</option>)}
              </select>
            </Field>
            <Field label="Model">
              <input value={form.model} onChange={e => setField("model", e.target.value)} placeholder="e.g. X5, Golf…" style={INPUT_STYLE}/>
            </Field>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <Field label="Year">
              <input type="number" value={form.year} onChange={e => setField("year", e.target.value)} placeholder="2020" min="1990" max="2026" style={INPUT_STYLE}/>
            </Field>
            <Field label="Type">
              <select value={form.type} onChange={e => setField("type", e.target.value)} style={INPUT_STYLE}>
